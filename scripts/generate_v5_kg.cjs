...
const CATEGORIES = [
  ...
  {
    id: 'securitization',
    name: 'Securitization & Structured Finance',
    color: '#96CEB4',
    topics: [
      ...
      { id: 'abs', title: 'ABS', subtitle: 'Asset-Backed Security', short_summary: 'A financial security backed by a loan, lease or receivables against assets other than real estate.', prereqs: ['tranching'], leads_to: ['rmbs', 'cmbs', 'cdo'] },
      { id: 'rmbs', title: 'RMBS', subtitle: 'Residential', short_summary: 'Residential Mortgage-Backed Securities are debt-based assets backed by the interest paid on residential loans.', prereqs: ['abs'], leads_to: [] },
      { id: 'cmbs', title: 'CMBS', subtitle: 'Commercial', short_summary: 'Commercial Mortgage-Backed Securities are fixed-income investment products backed by mortgages on commercial properties.', prereqs: ['abs'], leads_to: [] },
      { id: 'cdo', title: 'CDO', subtitle: 'Collateralized Debt', short_summary: 'A complex structured finance product backed by a pool of loans and other assets.', prereqs: ['abs'], leads_to: [] }
    ]
  },
  {
    id: 'derivatives',
...
CATEGORIES.forEach(cat => {
  ...
  cat.topics.forEach(topic => {
    nodes[topic.id] = {
      ...
      x: topic.isolated ? -999 : currentX,
      y: topic.isolated ? -999 : currentY,
      lines: topic.isolated ? [] : [cat.id],
      prerequisite_ids: topic.prereqs,
      leads_to_ids: topic.leads_to,
      aliases: [cat.name, cat.name.toLowerCase(), topic.title.toLowerCase()] // Added track name as alias
    };
    if (!topic.isolated) {
      lines[cat.id].stations.push(topic.id);
      currentX += 220; // Increased spacing for arrows
    }
  });
...
// Post-process: ensure reciprocal relationships
