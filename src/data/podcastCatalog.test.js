...
assert.deepEqual(getPodcastQueue('episodes', 'securitisation').map((track) => track.number), Array.from({ length: 13 }, (_, index) => index + 1));
assert.equal(getPodcastQueue('episodes', 'securitisation').at(0).title, 'The financing architecture');
assert.equal(getPodcastQueue('episodes', 'securitisation').some((track) => track.title === 'Payment Waterfall'), false);
